...
build-i386:
	docker build -t ${PROJECT_I386} --build-arg BASE_IMAGE=${IMAGE_I386} --build-arg VERSION=${VERSION} -f etc/docker/Dockerfile .

tag-i386:
	docker tag ${PROJECT_I386}:latest ${REPO}/${PROJECT}:${VERSION}

push-i386:
	docker push ${REPO}/${PROJECT_I386}:${VERSION}

build-armv7:
	docker build -t ${PROJECT_ARMV7} --build-arg BASE_IMAGE=${IMAGE_ARMV7} --build-arg VERSION=${VERSION} -f etc/docker/Dockerfile .
